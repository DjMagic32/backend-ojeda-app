...
class GastosMigrationTests(unittest.TestCase):
    def test_solo_crea_modelos_nuevos_con_campos_y_restricciones_equivalentes(self):
        ...
        migration_tree = ast.parse((root / 'store/migrations/0044_gastos.py').read_text())
        names = {}
        ...
        for node in model_tree.body:
            if isinstance(node, ast.Assign) and getattr(node.targets[0], 'id', None) in ('MONEDA_USD', 'MONEDA_VES', 'MONEDAS'):
        ...
        self.assertEqual(value(attrs['dependencies']), (('store', '0043_cuentas_por_pagar'),))
        self.assertEqual(len(attrs['operations'].elts), 2)
        for operation in attrs['operations'].elts:
            ...
            self.assertIn(name, ('Gasto', 'OperacionGasto'))
            fields = {}
            options = {}
