...
class PagosMigrationTests(unittest.TestCase):
    def test_solo_crea_modelos_nuevos_con_campos_y_restricciones_equivalentes(self):
        ...
        migration_tree = ast.parse((root / 'store/migrations/0043_cuentas_por_pagar.py').read_text())
        names = {'CuentaPorPagar': 'store.cuentaporpagar'}
        ...
        self.assertEqual(value(attrs['dependencies']), (('store', '0042_cuentas_por_cobrar'),))
        ...
        for operation in attrs['operations'].elts:
            ...
            self.assertIn(name, ('CuentaPorPagar', 'AbonoCuentaPorPagar', 'OperacionCuentaPorPagar'))
            fields = {}
            options = {}
