...
class CuentasMigrationTests(unittest.TestCase):
    def test_solo_crea_modelos_nuevos_con_campos_y_restricciones_equivalentes(self):
        ...
        migration_tree = ast.parse((root / 'store/migrations/0042_cuentas_por_cobrar.py').read_text())
        names = {'CuentaPorCobrar': 'store.cuentaporcobrar'}
        ...
        self.assertEqual(value(attrs['dependencies']), (('store', '0041_sesiones_caja'),))
        ...
        for operation in attrs['operations'].elts:
            ...
            self.assertIn(name, ('CuentaPorCobrar', 'AbonoCuentaPorCobrar', 'OperacionCuentaPorCobrar'))
            fields = {}
            options = {}
